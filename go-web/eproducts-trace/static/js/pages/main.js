// 厂家组件
const Farm = {
  // 接受分页属性，用来控制翻页
  props: ['currPage', 'pageSize'],
  data() {
    return {
      popup: false, // 创建电子产品弹窗
      eproductsList: [], // 电子产品信息
    }
  },
  computed: {
    // 当前页渲染数据
    renderList() {
      return this.eproductsList.slice((this.currPage - 1) * this.pageSize, this.currPage * this.pageSize)
    }
  },
  // 注册创建电子产品子组件
  components: {
    CreateEproducts
  },
  template: `
    <div>
      <!-- 创建电子产品按钮 -->
      <el-button type="primary" @click="popup = true;">新建电子产品</el-button>

      <!-- 退出按钮 -->
      <el-button type="danger" @click="$emit('logout', false)" class="button-logout">退出</el-button>

      <!-- 电子产品信息列表 -->
      <el-table
        :data="renderList"
        style="width: 100%"
        :default-sort = "{prop: 'date', order: 'descending'}"
      >
        <el-table-column
          prop="traceNumber"
          label="溯源码"
        >
        </el-table-column>
        <el-table-column
          prop="eproductsName"
          label="电子产品名称"
        >
        </el-table-column>
        <el-table-column
          prop="traceName"
          label="生产商"
        >
        </el-table-column>
        <el-table-column
          prop="date"
          label="生产时间"
          sortable
        >
        </el-table-column>
      </el-table>

      <!-- 创建电子产品子组件 -->
      <CreateEproducts :dialogFormVisible="popup" @popup="handlePopup" @confirmPopup="formSubmit" />
    </div>
  `,
  mounted() {
    // 挂载时获取厂家数据
    this.getData();
  },
  methods: {
    dateTime(time) {
      let newDate = new Date(parseInt(time));

      console.log("new date", newDate)
      let dateCollection = { y: newDate.getFullYear(), M: newDate.getMonth() + 1, d: newDate.getDate(), h: newDate.getHours(), m: newDate.getMinutes(), s: newDate.getSeconds() };
      for (let i in dateCollection) {
        if (dateCollection[i] < 10) dateCollection[i] = `0${dateCollection[i]}`;
      }
      let { y, M, d, h, m, s } = dateCollection;
      return `${y}-${M}-${d} ${h}:${m}:${s}`;
    },
    // 处理数据请求
    getData() {

      axios({
        method: 'get',
        url: `/producing`
      })
        .then(ret => {
          let _this = this;
          // 对数据进行处理
                this.eproductsList = ret.data
                  .map(item => ({
            ...item,
            date: _this.dateTime(item.timestamp),
            traceNumber: item.traceNumber,
            traceName: item.from,
            eproductsName: item.name
          }))
            .reverse();
                this.$emit('total', this.eproductsList.length);
        })
        .catch(err => {
          console.log(err)
        })
    },
    // 添加电子产品弹窗
    handlePopup(val) {
      this.popup = val;
    },
    // 添加电子产品提交
    formSubmit(val) {
      axios({
        method: 'post',
        url: '/produce', 
        data: {
          eproductsName: val.eproductsName,
          traceName: val.traceName,
          traceNumber: val.traceNumber.toString(),
          quality: val.quality.toString(),
          user: "0"
        }
      })
        .then(ret => {
          if (ret.data.ret !== 1) {
            // 已有溯源码校验
            if (ret.data.ret === 0 && ret.data.msg === 'traceNumber already exist') {
              this.$message({
                message: '该溯源码已存在，请重新创建',
                type: 'error',
                center: true
              });
            } else {
              this.$message({
                message: '提交失败',
                type: 'error',
                center: true
              });
            }
            return
          }
          this.$message({
            message: '提交成功',
            type: 'success',
            center: true
          });
          // 提交成功后重新获取数据
          this.getData();
        })
        .catch(err => {
          console.log(err)
        })
      this.popup = false;
    }
  }
}

// 中间商组件
const Agent = {
  // 和超市的职能差不多，放在 mixin 里面
  mixins: [mixin],
  data() {
    return {
      user: 1, // user: 1 为中间商
    }
  }
}

// 超市组件
const Mall = {
  // 和中间商的职能差不多，放在 mixin 里面
  mixins: [mixin],
  data() {
    return {
      user: 2, // user: 2 为超市
    }
  }
}

// 消费者组件
const Consumer = {
  // 溯源详情子组件注册
  components: {
    TraceDetail
  },
  data() {
    return {
      form: {
        traceNumber: '', // 输入的溯源码
      },
      eproductsDetail: '', // 溯源详细信息
      onSearch: false, // 搜索情况
    }
  },
  template: `
    <el-row style="height: 100%;" class="consumer">
      <!-- 左边为溯源查询 -->
      <el-col :span="7" style="height: 100%;">
        <div class="grid-content bg-purple">
          <el-form :model="form" ref="form">
            <h4>溯源码搜索</h4>
            <el-divider></el-divider>
            <el-form-item 
              label="请输入您想要查询的溯源码" 
              prop="traceNumber" 
            >
              <el-input v-model.number="form.traceNumber" type="textarea" :rows="3" autocomplete="off" @clear="onSearch = false;"></el-input>
            </el-form-item>
            <el-button type="primary" @click="onSubmit">查询</el-button>
          </el-form>
        </div>
      </el-col>

      <!-- 右边为溯源查询结果 -->
      <el-col :span="16">
        <div class="grid-content bg-purple-light">
          <div v-if ="!onSearch" class="consumer-tip">请在左侧查询栏中输入溯源码进行查询</div>  
          <div v-if ="onSearch && eproductsDetail.length === 0" class="consumer-tip">该溯源码无对应信息，请确认后重新查询</div>

          <!-- 溯源查询成功后才展示 -->
          <div v-if ="onSearch && eproductsDetail.length">
            <!-- 溯源详情组件， user: 3 为消费者 -->
            <TraceDetail :eproductsDetail="eproductsDetail" :user="3" />
          </div>  
        </div>
      </el-col>
      <el-col :span="1">
        <!--退出按钮 -->
        <el-button type="danger" @click="$emit('logout', false)" class="button-logout">退出</el-button>
      </el-col>
    </el-row>
  `,
  methods: {
    dateTime(time) {
      let newDate = new Date(time);
  
      let dateCollection = { y: newDate.getFullYear(), M: newDate.getMonth() + 1, d: newDate.getDate(), h: newDate.getHours(), m: newDate.getMinutes(), s: newDate.getSeconds() };
      for (let i in dateCollection) {
        if (dateCollection[i] < 10) dateCollection[i] = `0${dateCollection[i]}`;
      }
      let { y, M, d, h, m, s } = dateCollection;
      return `${y}-${M}-${d} ${h}:${m}:${s}`;
    },
    // 处理按溯源码查询
    onSubmit() {

      if (!this.form.traceNumber) {
        this.$message.error('请输入溯源码');
        return;
      }
      this.onSearch = true;
      axios({
        method: 'get',
        url: `/trace?traceNumber=${this.form.traceNumber}`
      })
        .then(ret => {
          this.eproductsDetail = ret.data
          this.eproductsDetail[0].produce_time = this.dateTime(parseInt(ret.data[0].produce_time))
          this.eproductsDetail[1].timestamp = this.dateTime(parseInt(ret.data[1].timestamp))
          this.eproductsDetail[2].timestamp = this.dateTime(parseInt(ret.data[2].timestamp))

          console.log( this.eproductsDetail );
        })
        .catch(err => {
          console.log(err);
        })
    }
  }
}