...
const mixin = {
  ...
  methods: {
    ...
    getData() {
      ...
      axios({
        method: 'get',
        url: `/${pathname}?user=${this.user}`
      })
      .then(ret => {
        this.eproductsList = ret.data
          .map(item => {
            let quality = parseInt(item.quality) === 0 ? '优质' : parseInt(item.quality) === 1 ? '合格' : '不合格'
            return ({
              ...item,
              date: this.dateTime(parseInt(item.timestamp)),
              quality,
            })
          })
          .reverse();
    ...
    addEproducts() {
      ...
      .then(ret => {
        ...
        if(this.user===1){
          ...
          this.eproductsDetail[0].produce_time = this.dateTime(parseInt(ret.data[0].produce_time))
          this.eproductsDetail[1].timestamp = this.dateTime(parseInt(ret.data[0].timestamp))
        }
